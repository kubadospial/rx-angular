...
export function addElementVariantFile(optionsSchema: {
  variant: string;
  project: string;
}): Rule {
  return (tree: Tree, context: SchematicContext) => {
    ...
    const project = parseApplicationProject(unsafeProject) as WorkspaceProject<
    ...
    const templatesPath =
      optionsSchema.variant === 'default'
        ? './files/variant-default'
        : './files/variant-custom';
